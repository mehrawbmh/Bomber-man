...
Bomb::Bomb(const sf::Vector2f &position, std::string textureFilePath) : MapElement(position) {
...
bool Bomb::isTimeToExplode() {
    return std::time(nullptr) >= this->plantTime + BOMB_EXPLOSION_TIME_SECONDS;
}

bool Bomb::shouldDestroy(sf::Vector2f objectPosition) {
    std::cout << this->destructionStartRange.x << ":" << this->destructionEndRange.x << "||" << destructionStartRange.y << ":" << destructionEndRange.y << std::endl;
